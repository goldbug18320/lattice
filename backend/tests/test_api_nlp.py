...
class TestDeployToRegion:
    def test_west_coast_command_returns_deploy_action(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        assert resp.status_code == 200
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["region"] == "west_coast"

    def test_east_coast_command_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "move all drones to east coast"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["region"] == "east_coast"

    def test_north_taiwan_command_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "redeploy assets to north taiwan"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["region"] == "north_taiwan"

    def test_south_taiwan_command_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "send units to south taiwan"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["region"] == "south_taiwan"

    def test_taipei_command_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy to taipei"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["region"] == "taipei"

    def test_strait_command_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "position drones in the strait"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["region"] == "taiwan_strait"

    def test_default_asset_filter_is_all(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        assert resp.json()["action"]["asset_filter"] == "all"

    def test_fpv_asset_filter_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "send fpv drones to west coast"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["asset_filter"] == "fpv"

    def test_altius_asset_filter_detected(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy altius to west coast"})
        assert resp.json()["action"]["type"] == "deploy_to_region"
        assert resp.json()["action"]["asset_filter"] == "altius"

    def test_execution_result_present(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        result = resp.json()["execution_result"]
        assert result is not None
        assert result["success"] is True

    def test_execution_tasks_swarms(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        result = resp.json()["execution_result"]
        assert result["swarms_tasked"] > 0
        assert result["drones_tasked"] > 0

    def test_execution_returns_region_and_filter(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        result = resp.json()["execution_result"]
        assert result["region"] == "west_coast"
        assert result["asset_filter"] == "all"

    def test_fpv_filter_only_tasks_fpv_swarms(self, client):
        resp = client.post("/api/nlp/command", json={"command": "send fpv drones to west coast"})
        result = resp.json()["execution_result"]
        assert all("FPV" in name for name in result["swarm_names"])

    def test_altius_filter_only_tasks_altius_swarms(self, client):
        resp = client.post("/api/nlp/command", json={"command": "deploy altius to west coast"})
        result = resp.json()["execution_result"]
        assert all("ALT" in name for name in result["swarm_names"])

    def test_swarms_are_patrolling_after_deploy(self, client):
        client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        swarms = client.get("/api/swarm/swarms").json()
        # All combat swarms should now be searching (en route to region)
        combat_swarms = [s for s in swarms if s["name"].startswith(("FPV", "ALT"))]
        assert all(s["status"] == "searching" for s in combat_swarms)

    def test_deploy_is_logged(self, client):
        client.post("/api/nlp/command", json={"command": "deploy all assets to the west coast"})
        history = client.get("/api/nlp/history").json()
        assert len(history) == 1
        assert history[0]["action_type"] == "deploy_to_region"

    def test_unknown_region_still_returns_deploy_action(self, client):
        """If no region keyword matched, deploy falls through to tactical/none — not a crash."""
        resp = client.post("/api/nlp/command", json={"command": "deploy all assets"})
        assert resp.status_code == 200
        # No region matched → falls through to none or tactical; should not crash
